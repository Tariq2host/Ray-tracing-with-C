...
public:
    ...
    bool intersect(const Rayon &r, Vector &P, Vector &N, double &t, Vector &color) override {
        ...
        if (texture) {
            ...
            int texY = std::min(textureHeight - 1, std::max(0, int(v * textureHeight)));
            int pixelIndex = (texX + texY * textureWidth) * 3;
            color[0]= texture[pixelIndex] / 255.0;
            color[1] = texture[pixelIndex + 1] / 255.0;
            color[2] = texture[pixelIndex + 2] / 255.0;
        } 
        else if (mapSphere_tri) 
        {
            Vector Pc = P - centre;
            double phi = atan2(Pc[2], Pc[0]);
            double theta = acos(Pc[1] / R);
            ...
            int triangleHeight = 55;
            double uScaled = u * triangleWidth;
            double vScaled = v * triangleHeight;
            int i = (int)uScaled;
            int j = (int)vScaled;
            ...
            if ((i + j) % 2 == 0) {
                color = (fu + fv < 1) ? Vector(1, 0.2, 0.4) : lightBlue;
                // color = (fu + fv < 1) ? Vector(1, 1, 1) : Vector(0,0,0);
            } else {
                color = (fu + fv < 1) ? lightBlue : Vector(1, 0.2, 0.4);
